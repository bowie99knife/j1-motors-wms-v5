...
export default function JobEditor(){
  const {id}=useParams();const nav=useNavigate();const [form,setForm]=useState<any>(blank);const [message,setMessage]=useState("");const [photos,setPhotos]=useState<any[]>([]);
  ...
  async function save(e:FormEvent){e.preventDefault();const payload={...form,year:form.year?Number(form.year):null,odometer:form.odometer?Number(form.odometer):null};const res=id==="new"?await supabase.from("job_cards").insert(payload).select("id").single():await supabase.from("job_cards").update(payload).eq("id",id);if(res.error)return setMessage(res.error.message);nav("/dashboard/jobs")}
  async function upload(files:FileList|null){if(!files||!id||id==="new")return setMessage("Save the job card before uploading photos.");for(const file of Array.from(files)){const path=`${id}/${crypto.randomUUID()}-${file.name}`;const up=await supabase.storage.from("job-photos").upload(path,file);if(!up.error)await supabase.from("job_photos").insert({job_id:id,storage_path:path,caption:file.name})}const {data}=await supabase.from("job_photos").select("*").eq("job_id",id);setPhotos(data||[])}
  return <form className="panel formGrid" onSubmit={save}>
    <h1 className="wide">{id==="new"?"New Job Card":form.job_number}</h1>
    {["status","customer_name","customer_phone","customer_email","rego","make","model","year","odometer","engine_code","transmission"].map(k=><label key={k}>{k.replaceAll("_"," ")}<input value={form[k]||""} onChange={e=>setForm({...form,[k]:e.target.value})}/></label>)}
